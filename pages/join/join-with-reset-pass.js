...
export default function Join() {
  ...
  const [forgotPassToast, setForgotPassToast] = useState(false);

  const router = useRouter();
  const { referral } = router.query;
  const { doesStudioExist } = useStudios();

  const { signUp, signIn, noPassword, user } = useAuth();
  ...
  const handleForgotPassword = async (event) => {
    ...
    try {
      setLoading(true);
      const { error } = await noPassword(email, {
        redirectTo: "https://www.arti.my/profile/reset-password",
      });
            ...
            <Box margin={"small"}>
              {newUser ? (
                ...
                <>
                  ...
                  <Text margin={fieldMargin}>
                    <Anchor onClick={() => setForgotPassword(!forgotPassword)}>
